...
# note: python allows negative indexing, so we don't need to offset diff
#
...
def solve(cur):
    ...
    if len(graph[cur]) == 1:  # only 1 child to transition from
        if color[cur] == 1:  # transition to white node: +1
            for i in range(-N + 1, N):
                dp[cur][i] = min(dp[cur][i], dp[adj][i - 1])
        else:  # transition to black node: -1
        ...
        for adj in graph[cur]:  # transition 1: cut off 1 branch
            if color[cur] == 1:  # transition to white node: +1
                for i in range(-N + 1, N):
                    dp[cur][i] = min(dp[cur][i], dp[adj][i - 1] + 1)
            else:  # transition to black node: -1
                for i in range(-N, N - 1):
                    dp[cur][i] = min(dp[cur][i], dp[adj][i + 1] + 1)
